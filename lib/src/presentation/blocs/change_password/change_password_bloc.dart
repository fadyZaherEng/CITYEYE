...
class ChangePasswordBloc
  ...
  ChangePasswordBloc(
    ...
    on<ChangePasswordValidationEvent>(_onChangePasswordValidationEvent);
  ...
  FutureOr<void> _onChangePasswordValidationEvent(
      ChangePasswordValidationEvent event, Emitter<ChangePasswordState> emit) {
      ...
      for (var element in validationsState) {
        if (element == ChangePasswordValidationState.oldPasswordEmpty) {
          ...
          emit(ConfirmPasswordNotMatchNewPasswordFormatState(
            confirmPasswordValidatorMessage: S.current.passwordDoesntMatch,
          ));
        }
      }
    } else {
  ...
  FutureOr<void> _onChangePasswordEvent(
    ...
    DataState dataState = await _changePasswordUseCase(ChangePasswordRequest(
      oldPassword: event.oldPassword,
      newPassword: event.newPassword,
      confirmPassword: event.confirmPassword,
    ));
    if (dataState is DataSuccess) {
      emit(ChangePasswordSuccessState(message: dataState.message ?? ""));
    } else {
      emit(ChangePasswordErrorState(errorMessage: dataState.message ?? ""));
    }
